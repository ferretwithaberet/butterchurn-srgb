...
function parseSRGBValue(
  value: string,
  type: string,
): number | boolean | string | null;
function parseSRGBValue(value: string, type: string) {
  if (!["number", "hue", "boolean", "color", "text", "list"].includes(type))
    return null;
...
const setupProperties = (overrideValues?: Partial<SignalRGBProperties>) => {
  ...
  void [...metaTags].forEach((metaTag) => {
    ...
    void ((window as any)[name] =
      (overrideValues as any)?.[name] ?? parsedValue);
  });
};
...
const mockAudioEngine = (random: boolean = false) => {
  ...
  window.engine = {
    audio: {
      ...
      freq: Array.from({ length: 200 }).map(() =>
        random ? generateRandomInt(0, 255) : 0,
      ),
    },
  };
};
