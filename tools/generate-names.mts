...
// Presets TS file
let tsFileContent = '/* eslint-disable @typescript-eslint/dot-notation */\n';
presets.forEach((preset, index) => {
  tsFileContent += `import preset${index} from 'butterchurn-presets/presets/converted/${preset.replace(/'/g, String.raw`\'`)}.json';\n`;
});
tsFileContent += '\nconst presets: Record<string, any> = {};\n';
presets.forEach((preset, index) => {
  tsFileContent += `presets['${preset.replace(/'/g, String.raw`\'`)}'] = preset${index};\n`;
});
